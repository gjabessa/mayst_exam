...
public class Problem1 {
	...
	public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
		//implement
		return Stream.concat(list1.stream().filter(n -> !list2.contains(n)),list2.stream().filter(n -> !list1.contains(n))).collect(Collectors.toList());
	...
	public static List<String> getAllFairfieldCustomers(List<Customer> list) {
		//implement
		return list.stream().filter(n -> n.getCity()=="Fairfield").map(c -> c.getName()).collect(Collectors.toList());
	}
